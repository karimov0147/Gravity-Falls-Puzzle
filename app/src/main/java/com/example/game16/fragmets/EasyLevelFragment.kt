...
import com.example.game16.databinding.FragmentEasyLevelBinding
...
class EasyLevelFragment : Fragment() , View.OnClickListener {

    lateinit var binding : FragmentEasyLevelBinding
    var emptyPosition = 8
    var counter = 1
    var isPlaying = false
    var isresume = false
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_easy_level, container, false)
        binding = FragmentEasyLevelBinding.bind(view)
        ...
        btn = arrayListOf<AppCompatButton>(
            ...
            binding.btn7,binding.btn8
        )

        numbers = arrayListOf<Int>(1,2,3,4,5,6,7,8)
    ...
    override fun onClick(clicked: View) {
        if (clickable){
            ...
            if (emptyPosition - pos == 1 && emptyPosition % 3 != 0) {
                clicked.tag = emptyPosition
                emptyPosition = pos
                binding.textView.text = "Step : ${counter++}"
                startAnim(clicked, "x", clicked.x + (clicked.width+convertDpToPixel(12f,mContext)))
            } else if (emptyPosition - pos == -1 && pos % 3 != 0) {
                clicked.tag = emptyPosition
                emptyPosition = pos
                binding.textView.text = "Step : ${counter++}"
                startAnim(clicked, "x", clicked.x - (clicked.width+convertDpToPixel(12f,mContext)))
            } else if (emptyPosition - pos == 3) {
                clicked.tag = emptyPosition
                emptyPosition = pos
                binding.textView.text = "Step : ${counter++}"
                startAnim(clicked, "y", clicked.y + (clicked.height+convertDpToPixel(12f,mContext)))
            } else if (emptyPosition - pos == -3) {
                clicked.tag = emptyPosition
                emptyPosition = pos
                binding.textView.text = "Step : ${counter++}"
...
//}
            var obj = Score(name,step, text.toString() , "Easy")
